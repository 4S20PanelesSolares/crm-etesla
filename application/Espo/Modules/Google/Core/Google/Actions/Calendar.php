...
class Calendar extends Base
{
                ...
                if (isset($result['nextPageToken'])) {
                    ...
                    if (is_array($lastIdArr) && !empty($lastIdArr[count($lastIdArr) - 1])) {
                        try {
                            ...
                            $GLOBALS['log']->error('Google Calendar Synchronization: Last recurrent id is ' . $lastId . ". " . $e->getMessage());
                        }
                    }
                    $this->getEventManager()->updateRecurrentEvent($recurrentEvent['id'], $result['nextPageToken'], $lastDateStr);
                } else if (isset($result['nextSyncToken'])) {
                    $this->getEventManager()->removeRecurrentEventFromQueue($recurrentEvent['id']);
                }
            } catch (\Exception $e) {
    ...
    public function run($calendar, $externalAccount)
    {
        if (!$this->getAcl()->checkScope('GoogleCalendar')) {
            $GLOBALS['log']->info("Google Calendar Synchronization: Access Forbidden for user {$calendar->get('userId')}");
            return false;
        }
        try {
            ...
            $GLOBALS['log']->error("Google Calendar Synchronization: Error when calendar synchronization is running. GoogleCalendarUser Id {$calendar->id}. Message: {$e->getMessage()}");
        }
        return false;
    }
}
