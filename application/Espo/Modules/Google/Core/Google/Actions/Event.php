...
class Event extends Base
{
    ...
    public function updateGoogleEvent($event, $withCompare)
    {
       $googleEvent = $this->retrieveGoogleEvent($event['googleCalendarEventId']);
        ...
        foreach ($this->googleEspoPairs as $googleField => $espoField) {
            ...
            if (!array_key_exists($espoField, $event)) continue;
        ...
        if (!$this->syncParams['dontSyncEventAttendees'] && $event['attendees'] !== null) {
            ...
            foreach ($event['attendees'] as $espoAttendee) {
                ...
                if (empty($emailAddress) && isset($espoAttendee['emailData']) && isset($espoAttendee['emailData'][0]) && is_object($espoAttendee['emailData'][0])) {
        ...
        if (!empty($changedFields)) {
            $res = $this->getClient()->updateEvent($this->getCalendarId(), $googleEvent->getId(), $googleEvent->build());
            if (!$res) {
                $this->getGoogleCalendarRepository()->storeEventRelation($event['scope'], $event['id'], '', 'FAIL');
            } else {
                return true;
            }
        }
    ...
    public function updateEspoEvent($event, $withCompare = true)
    {
        ...
        foreach ($espoEvents as $espoEvent) {
            ...
            if (!$eventIsNew && $espoEvent->get('googleCalendarId') != $this->syncParams['calendar']->get('googleCalendarId')) {
            ...
            foreach ($this->googleEspoPairs as $googleField => $espoField) {
                if (!$espoEvent->hasAttribute($espoField)) continue;
            ...
            if ($eventIsNew) {
                ...
                $espoAttendees = $this->getGoogleCalendarRepository()->getEventAttendees($espoEvent->getEntityType(), $espoEvent->id);
    ...
    public function convertEventToGoogle($espoEvent)
    {
        ...
        if (!empty($espoEvent['attendees'])) {
            foreach($espoEvent['attendees'] as $attendee) {
                if ($this->syncParams['dontSyncEventAttendees'] && in_array($attendee['entityType'], ['Contact', 'Lead'])) {
                    continue;
                }
    ...
    {
       //old
       return $this->getGoogleCalendarRepository()->addRecurrentEventToQueue(
