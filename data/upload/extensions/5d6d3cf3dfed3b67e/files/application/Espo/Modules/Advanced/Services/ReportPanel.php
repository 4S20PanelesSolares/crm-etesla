...
class ReportPanel extends \Espo\Services\Record
{
    ...
    protected function afterCreateEntity(\Espo\ORM\Entity $entity, $data)
    {
        $this->rebuild($entity->get('entityType'));
    }

    protected function afterUpdateEntity(\Espo\ORM\Entity $entity, $data)
    {
        $this->rebuild($entity->get('entityType'));
    }

    protected function afterDeleteEntity(\Espo\ORM\Entity $entity)
    {
        $this->rebuild($entity->get('entityType'));
    }

    public function loadAdditionalFields(\Espo\ORM\Entity $entity)
    {
    ...
    public function rebuild($specificEntityType = null)
    {
        ...
        $language = new \Espo\Core\Utils\Language('en_US', $this->getInjection('fileManager'), $this->getMetadata());
        ...
        foreach ($entityTypeList as $entityType) {
            ...
            foreach ($typeList as $type) {
                ...
                $reportPanelList = $this->getEntityManager()->getRepository('ReportPanel')->where([
                    'isActive' => true,
                ...
                foreach ($reportPanelList as $reportPanel) {
                    if (!$reportPanel->get('reportId')) continue;
    ...
    public function run($type, $id, $parentType, $parentId, $params = null)
    {
        ...
        if (!$reportPanel) throw new NotFound('Report Panel not found.');
        if (!$this->getAcl()->checkScope($reportPanel->get('reportEntityType'))) throw new Forbidden();

        if (!$parentId || !$parentType)  throw new BadRequest();
        $parent = $this->getEntityManager()->getEntity($parentType, $parentId);

        if (!$parent) throw new NotFound();

        if (!$this->getAcl()->checkEntity($parent, 'read')) throw new Forbidden();
        if (!$reportPanel->get('reportId')) throw new Error('Bad Report Panel.');
        ...
        if (count($teamIdList) && !$this->getUser()->isAdmin()) {
            ...
            if (!$isInTeam) throw new Forbidden("Access denied to Report Panel.");
        }
        ...
        if ($type === 'List' && $report->get('type') === 'JointGrid') {
            if (empty($params['subReportId'])) throw new BadRequest();
            $joinedReportDataList = $report->get('joinedReportDataList');
            if (empty($joinedReportDataList)) throw new Error();
        ...
        if ($report->get('type') === 'JointGrid') {
            ...
            foreach ($report->get('joinedReportDataList') as $subReportItem) {
                $subReport = $this->getEntityManager()->getEntity('Report', $subReportItem->id);
                if (!$subReport) throw new Error('Sub report not found.');
        ...
        if ($type === 'Grid') {
            return $this->getServiceFactory()->create('Report')->run($report->id, $where, $params, ['skipRuntimeFiltersCheck' => true], $this->getUser());
        }
        if ($type === 'List') {
            return $this->getServiceFactory()->create('Report')->run($report->id, $where, $params, ['skipRuntimeFiltersCheck' => true], $this->getUser());
        }
    ...
    protected function getWhere(Entity $parent, Entity $report)
    {
        ...
        foreach ($filterList as $item) {
            ...
            if ($linkType === 'belongsTo' || $linkType === 'hasMany') {
                $foreignEntityType = $this->getMetadata()->get(['entityDefs', $entityType, 'links', $field, 'entity']);
                if ($foreignEntityType !== $parent->getEntityType()) continue;
                ...
                $entityTypeList = $this->getMetadata()->get(['entityDefs', $entityType, 'fields', $field, 'entityList'], []);
                if (!in_array($parent->getEntityType(), $entityTypeList)) continue;
        ...
        if (!$where) {
            ...
            foreach ($linkList as $link) {
                ...
                if ($linkType === 'belongsToParent') {
                    $entityTypeList = $this->getMetadata()->get(['entityDefs', $entityType, 'fields', $link, 'entityList'], []);
