...
abstract class Base
{
    ...
    public function process($entity, $action, $createdEntitiesData = null, $variables = null, $bpmnProcess = null)
    {
        ...
        $GLOBALS['log']->debug('Workflow\Actions: End ['.$action->type.'] with cid ['.$action->cid.'] for entity ['.$entity->getEntityType().', '.$entity->id.'].');

        if (!$result) {
            $GLOBALS['log']->debug('Workflow['.$this->getWorkflowId().']: Action failed [' . $action->type . '] with cid [' . $action->cid . '].');
        }
    }
