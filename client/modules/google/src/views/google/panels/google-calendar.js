...
Espo.define('google:views/google/panels/google-calendar', 'view', function (Dep) {
    ...
    return Dep.extend({
        ...
        setupFields: function () {
            ...
            var eventOptions = Object.keys(scopes).filter(function (scope) {
                if (version.length == 3 &&
                    (parseInt(version[0]) < 4 ||
                    parseInt(version[0]) == 4 && parseInt(version[1]) < 5)) {
                    if (scope != 'Call' && scope != 'Meeting') return;
                }
                if (scope === 'Email') return;
                if (scopes[scope].disabled) return;
                if (!scopes[scope].object) return;
                if (!scopes[scope].entity) return;
                if (!scopes[scope].activity) return;
                return true;
