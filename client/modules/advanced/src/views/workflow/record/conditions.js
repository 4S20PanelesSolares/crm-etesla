...
Espo.define('advanced:views/workflow/record/conditions', ['view', 'advanced:workflow-helper'], function (Dep, Helper) {
        ...
        addCondition: function (conditionType, field, data, isNew) {
            ...
            if (~field.indexOf('.')) {
                if (field.indexOf('created:') === 0) {
                    ...
                    fieldType = this.getMetadata().get(['entityDefs', overridenEntityType, 'fields', overridenField, 'type']) || 'base';
                } else {
                    var arr = field.split('.');
                    foreignField = arr[1];
                    link = arr[0];
            ...
            if (isCreatedEntity) {
                ...
                label = this.translate(link, 'links', this.entityType) + '.' + this.translate(foreignField, 'fields', foreignEntityType);
            ...
            var removeLinkHtml = this.readOnly ? '' : '<a href="javascript:" class="pull-right" data-action="removeCondition" data-id="'+id+'"><span class="fas fa-times"></span></a>';
